...
export async function converse({ message, question, profile, history = [] }) {
  ...
  const passages = question ? retrieve(question.questionText ?? question.text, { limit: 8, charBudget: 2500 }) : [];
...
function fallback(message, question) {
  ...
  const isAnswer = !trivial.test(message.trim()) && message.trim().length > 3;
  return {
