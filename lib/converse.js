...
/**
 * Groq meters tokens per DAY per model, and per ORGANISATION — a fresh API key
 * in the same org inherits the same exhausted budget. So when a model's daily
 * ceiling is reached the answer is not a new key, it is a different model.
 * These are tried in order; each has its own daily pool.
 */
...
${passagesForPrompt(passages)}`;
  ...
  try {
    ...
    let res;
    // Try each model in turn. A per-minute limit is worth waiting out on the
    // same model; a per-DAY limit means this model is finished until midnight
    // UTC, so move to the next one rather than burning retries on it.
    let modelUsed = FALLBACK_MODELS[0];
    outer: for (const candidate of FALLBACK_MODELS) {
      modelUsed = candidate;
      ...
      body: JSON.stringify({
        ...
        // 404 = model gone or not on this key; 400 = it rejected a parameter we
        // sent. Either way this model cannot serve the turn, so try the next
        // rather than reporting the whole service as unreachable.
        if (res.status === 404 || res.status === 400) break; // next model
        if (res.status !== 429) break outer;
        // Read the body once to tell a per-day ceiling from a per-minute burst.
        const detail = await res.clone().text();
        if (detail.includes('per day') || detail.includes('TPD')) break; // next model
