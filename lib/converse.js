...
${passagesForPrompt(passages)}`;
  ...
  const standing = evidenceKind(question, passages);
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: 'system', content: `PROGRESS SO FAR: ${progress}` },
          {
            role: 'system',
            content:
              'HARD RULE FOR THIS TURN: if their message is a bare yes/no or under about ' +
              'five words, your ENTIRE reply is ONE short question asking for the next ' +
              'missing detail. Do NOT compose an answer on their behalf, do NOT summarise ' +
              'what their documents say, do NOT cite anything. They are answering; you are ' +
              'asking. Composing their answer for them puts words in a vendor\'s mouth and ' +
          ...
          },
          {
            ...
            content:
              standing === 'attested'
                ? 'IMPORTANT FOR THIS TURN: every document you have on this question is a policy, ' +
                  'plan or auditor opinion. None of them shows the control actually running. Your ' +
                  'reply MUST say that in plain words — their policy states it, you have nothing ' +
                  'showing it in force — and then ask for ONE specific thing that would settle it: ' +
                  'a date, a scope, an owner, a scan, an export or a log. Do not accept the policy ' +
                  'as the answer.'
                : standing === 'observed'
                ? 'FOR THIS TURN: you have an observed record here — a scan, log or register. Ground ' +
                  'your reply in it rather than in policy.'
                : 'FOR THIS TURN: nothing in their documents bears on this. Do not guess — ask.',
          },
 ...
 * what stops that.
 */
/** 'observed' | 'attested' | 'none' — what we are standing on for this question. */
function evidenceKind(question, passages) {
  const kinds = new Set([
    ...(question?.evidence ?? []).map((e) => e.evidenceType),
    ...passages.map((p) => p.evidenceType),
  ]);
  if (kinds.has('observed')) return 'observed';
  return kinds.size ? 'attested' : 'none';
}
...
function evidenceStanding(question, passages) {
  ...
  if (kinds.has('observed')) {
    return 'You have at least one OBSERVED record here — a scan, log or register. Use it.';
  }
  if (kinds.size) {
    return 'ATTESTED ONLY — every passage is a policy, plan or opinion. Nothing here shows the control in force. Say that, and ask for the record, date, scope or owner that would.';
  }
  return 'Nothing bears on this question. Do not guess; ask.';
}
