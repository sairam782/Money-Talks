...
for (const doc of DOCS) {
  doc.text.split('\n').forEach((line, i) => {
    ...
    if (text.length < 40) return; // headings and stray cells carry no evidence
    PARAS.push({
      docId: doc.docId,
      docName: doc.docName,
...
export function retrieve(question, { limit = 40, charBudget = 13000 } = {}) {
  ...
  const perDoc = {};
  const push = (p) => {
    if (picked.length >= limit || chars + p.text.length > charBudget) return false;
  ...
  return picked.map(({ docId, docName, evidenceType, date, text, score, whole }) => ({
    docId, docName, evidenceType, date, text, score: Number(score.toFixed(2)), whole: !!whole,
  }));
}
