  ...
  for (const q of QUESTIONS) {
    ...
    entries[q.id] = {
      ...
      followUpQuestion:
        a?.followUpQuestion ??
        (a?.status === 'unknown' || !a
          ? `Not covered in your documents — can you confirm? ${q.text}`
          : null),
      history: a
...
export function nextOpenQuestion(profile) {
  ...
  return (
    // Conflicts come first — a contradiction matters more than a gap. But they
    // must respect askedQuestions like everything else: without that, an
    // unresolved conflict is offered again on every single turn and the
    // interview cannot move past it. Raise it once; if they do not rule on it,
    // carry on and leave it open in the questionnaire.
    all.find((a) => a.status === 'conflict' && !profile.askedQuestions.includes(a.questionId)) ??
