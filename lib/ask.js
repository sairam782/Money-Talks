...
/** See lib/converse.js: daily quota is per model AND per organisation. */
...
PROVENANCE. Each document in the passages carries the line on which it names its owner —
"Company Name: ...", a title, a client field. Treat that as evidence like any other. If a
document filed as Regodit's evidence names a DIFFERENT company, say so and quote the line:
a continuity plan belonging to another organisation does not establish that Regodit has
one, and that is a "conflict", not an "answered". The same applies when the assessor and
the assessed party are the same, or when a document is undated or expired. Answering
"whose document is this?" is always possible when an identity line is in front of you.
...
export async function askCorpus(question, { limit = 16, charBudget = 6500, retried = 0 } = {}) {
  ...
  try {
    ...
    for (const candidate of FALLBACK_MODELS) {
      ...
      body: JSON.stringify({
        ...
        // unreachable.
        reasoning_effort: modelUsed.includes('gpt-oss') ? 'low' : 'none',
      ...
      // Out of daily budget, or this model cannot serve the request at all —
      // either way, try the next one. A per-minute limit falls through to the
      // wait-and-retry below, which is the right response to asking too fast.
      if (res.status === 404 || res.status === 400 || detail.includes('per day') || detail.includes('TPD')) continue;
      break;
    }
    ...
    if (res.status === 429 && retried < 3) {
      const wait = Number(res.headers.get('retry-after')) || 6 * (retried + 1);
      await new Promise((r) => setTimeout(r, Math.min(wait + 1, 12) * 1000));
      return askCorpus(question, { limit, charBudget, retried: retried + 1 });
    }
