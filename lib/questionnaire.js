...
/**
 * The corpus is deliberately not in the repository: it holds a live penetration
 * test, access records naming real staff, and a W-9. So a build — or a clone —
 * can legitimately have no data/ at all, and must not crash for it. Absent data
 * yields an empty questionnaire, and the UI says so.
 */
const QUESTIONS_PATH = path.join(process.cwd(), 'data', 'questions.json');
const raw = fs.existsSync(QUESTIONS_PATH)
