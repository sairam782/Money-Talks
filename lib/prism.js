...
/**
 * One trace per free-form question — the ones that are not in the workbook
 * (lib/ask.js). Traced on the same session as the interview, so a reviewer's
 * side questions appear in the trajectory alongside the 66. Never awaited.
 */
export function traceAsk({ question, result, runId }) {
  return emitTrace({
    model: result.model,
    input: question,
    output: result.answer ?? `(${result.status})`,
    latencyMs: result.latencyMs,
    sessionId: runId,
    metadata: {
      kind: 'off_questionnaire',
      status: result.status,
      evidenceType: result.evidence?.[0]?.evidenceType ?? null,
      citationCount: result.evidence?.length ?? 0,
      rejectedCount: result.rejectedEvidence?.length ?? 0,
      retrievedFrom: result.retrievedFrom?.length ?? 0,
...
/**
 * One trace per spoken reply or transcribed recording (lib/voice.js). Voice
 * does not decide anything — it renders a reply that was already produced under
 * the citation rule, or turns a recording into the text the user would have
 * typed — but both are model calls, and an unwired model call is invisible in
 * the dashboard (CLAUDE.md standing rule). Never awaited.
 */
export function traceVoice({ direction, model, input, output, latencyMs, runId, metadata = {} }) {
  return emitTrace({
    model,
    input,
    output,
    latencyMs,
    sessionId: runId,
    metadata: { kind: 'voice', direction, ...metadata },
  }).catch(() => {}); // tracing must never break the app
}
